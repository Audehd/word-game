...
export const updateLabel = createAsyncThunk("labels/updateLabel", async ({ id, label }) => {
  const res = await axios.put(`${API_URL}/${id}`, label);
  return res.data.data;
});

export const deleteLabel = createAsyncThunk("labels/deleteLabel", async (id) => {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(updateLabel.fulfilled, (state, action) => {
        const index = state.items.findIndex((label) => label._id === action.payload._id);
        if (index !== -1) state.items[index] = action.payload;
      })
      .addCase(deleteLabel.fulfilled, (state, action) => {
        state.items = state.items.filter((label) => label._id !== action.payload._id);
      });
  }
});
